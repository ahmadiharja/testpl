...
<table>
    <tr>
        <td><b>Report:</b></td>
        <td colspan="4" style="color: #4a86e8;"><b>All Displays</b></td>
    </tr>
    <tr>
        <!-- Display Model	Display Serial Number	Facility	Workstation	Workgroup -->
        <td class="odd subhead"><b>Display</b></td>
        <td class="even subhead"><b>Inventory Number</b></td>
        <td class="even subhead"><b>Workstation</b></td>
        <td class="even subhead"><b>Workgroup</b></td>
        <td class="even subhead"><b>Facility</b></td>
        <td class="even subhead"><b>Status</b></td>

    </tr>
    @php $i=0; @endphp
    @foreach($data as $d)
    @php
    if($d->manufacturer==null) $d->manufacturer='';
    $display=$d->manufacturer . '  ' . $d->model.' ('.$d->serial.')';

    $status='OK';
    if($d->status!='1') $status='FAILED';
    @endphp
        <tr>
            ...
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $d->workstation->workgroup->facility->name }}</td>
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $status }}</td>
        </tr>
        @php $i++ @endphp
    @endforeach
</table>