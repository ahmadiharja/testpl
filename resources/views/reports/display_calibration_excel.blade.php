...
@endphp
<!--<img src="" style="max-width:150px; margin-bottom:20px; margin-right:30px;">
<img src="" style="max-width:200px; margin-bottom:20px;">-->

<table>
    <tr>
        <td><b>Report:</b></td>
        <td colspan="4" style="color: #4a86e8;"><b>Display Calibration</b></td>
    </tr>
    <tr>
    ...
    @endphp
        <tr>
            ...
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $d->display->workstation->workgroup->facility->name }}</td>
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $d->taskType->title }}</td>
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $d->ScheduleType->title }}</td>
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $due_date }}</td>
        </tr>
        @php $i++ @endphp
    @endforeach
</table>