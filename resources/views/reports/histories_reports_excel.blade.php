...
<table>
    <tr>
        <td><b>Report:</b></td>
        <td colspan="4" style="color: #4a86e8;"><b>Histories</b></td>
    </tr>
    <tr>
    ...
    $result='OK';
    if($d->result=='3') $result='FAILED';
    if($d->result=='4') $result='SKIPPED';
    if($d->result=='5') $result='CANCELLED';
    @endphp
        <tr>
            ...
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ date('Y-m-d H:i:s', $d->time) }}</td>
            <td class="content {{ $i % 2 == 0 ? "trodd": "treven" }}">{{ $result }}</td>
        </tr>
        @php $i++ @endphp
    @endforeach
</table>