...
<head>
    ...
    <title>{{ $title ?? 'Forgot Password' }} | {{ $settings['Site name'] ?? 'PerfectLum' }}</title>
        ...
        <div class="hidden flex-1 lg:block">
            <div class="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#161820] p-10 shadow-2xl">
                ...
                <div class="relative z-10 flex h-[520px] flex-col justify-between">
                    ...
                    <div>
                        <p class="mb-3 text-sm font-semibold uppercase tracking-[0.28em] text-sky-300">Account Recovery</p>
                        <h1 class="max-w-lg text-4xl font-extrabold leading-tight">Reset akses akun tanpa kembali ke UI lama.</h1>
                        <p class="mt-4 max-w-md text-sm text-white/65">Masukkan email atau username, lalu sistem akan mengirim tautan reset password ke alamat email yang terdaftar.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="w-full lg:max-w-md" x-data="forgotPasswordForm()">
            <div class="rounded-[2rem] border border-white/10 bg-[#161820] p-8 shadow-2xl">
                <div class="mb-8">
                    ...
                    <h2 class="mt-6 text-3xl font-extrabold tracking-tight">Forgot password</h2>
                    <p class="mt-2 text-sm text-white/55">We will email a reset link if the account exists.</p>
                </div>

                <form @submit.prevent="submit" class="space-y-5">
                    @csrf
                    <div>
                        <label for="email" class="mb-2 block text-sm font-medium text-white/70">Email / Username</label>
                    ...
                    <button type="submit" :disabled="loading"
                        ...
                        <span x-text="loading ? 'Please wait...' : 'Send Reset Link'"></span>
                    </button>
                </form>
            </div>
        </div>
                ...
                async submit() {
                    ...
                    try {
                        ...
                        if (data.success === 'passwords.user') {
                            this.error = "We can't find a user with this e-mail address.";
                        } else if (!data.success) {
                            this.error = data.msg || 'Unable to send reset link.';
                        } else {
                            this.success = true;
                        }
                    } catch (error) {
